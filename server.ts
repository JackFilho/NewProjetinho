 ...
 * This ensures all date calculations use Brazil timezone consistently
 */
...
// NOTA: Este endpoint está DUPLICADO e foi DESATIVADO
// O endpoint principal está em server/routes.ts linha ~4840
// Comentado para evitar processamento duplicado de webhooks
// Webhook endpoint for WhatsApp integration with AI agent
app.post('/api/webhook/whatsapp/:instanceName', async (req, res) => {
  // DESATIVADO - endpoint duplicado, use server/routes.ts
  return res.status(200).json({ received: true, processed: false, reason: 'Endpoint duplicado desativado - use routes.ts' });
...
🚨🚨🚨 REGRA CRÍTICA - CÁLCULO DE SOBREPOSIÇÃO DE HORÁRIOS 🚨🚨🚨

ANTES de confirmar QUALQUER horário, você DEVE calcular se há sobreposição:

PASSO 1: Identifique a DURAÇÃO do serviço escolhido (exemplo: 90 minutos)
PASSO 2: Calcule o HORÁRIO DE TÉRMINO = Horário Solicitado + Duração do Serviço
PASSO 3: Verifique se há SOBREPOSIÇÃO com horários ocupados

EXEMPLO PRÁTICO DE CÁLCULO:
→ Serviço: Limpeza de Pele (90 minutos)
→ Horário solicitado: 09:00
→ CÁLCULO: 09:00 + 90min = 10:30 (horário de término)
→ Horários ocupados: "OCUPADO 09:30-10:00 (30min)"
→ ANÁLISE: O período 09:00-10:30 SOBREPÕE com 09:30-10:00
→ CONCLUSÃO: ❌ NÃO DISPONÍVEL - há conflito!

→ Serviço: Massagem (30 minutos)
→ Horário solicitado: 10:30
→ CÁLCULO: 10:30 + 30min = 11:00 (horário de término)
→ Horários ocupados: "OCUPADO 09:30-10:00 (30min)"
→ ANÁLISE: O período 10:30-11:00 NÃO sobrepõe com 09:30-10:00
→ CONCLUSÃO: ✅ DISPONÍVEL - sem conflito!

REGRA DE SOBREPOSIÇÃO:
Há sobreposição SE:
- O horário de INÍCIO solicitado for ANTES do FIM de um período ocupado E
- O horário de TÉRMINO calculado for DEPOIS do INÍCIO de um período ocupado

NUNCA confirme um horário sem fazer este cálculo primeiro!
Se houver sobreposição, INFORME o cliente e sugira horários alternativos.
...
- Após o nome, peça PRIMEIRO a data desejada (em etapas separadas):
  ...
  3. 🚨 IMPORTANTE - AO PERGUNTAR O HORÁRIO: SEMPRE informe os horários ocupados nesse momento
     * Exemplo: "Qual horário você prefere? Lembrando que os horários disponíveis são das 09:00 às 18:00, exceto às 09:30 que já está ocupado. Os horários geralmente são de 30 em 30 minutos (09:00, 09:30, 10:00, 10:30, etc)."
     * Consulte SEMPRE a seção "DISPONIBILIDADE REAL DOS PROFISSIONAIS POR DATA" antes de perguntar
     * A seção mostra formato: "OCUPADO 09:30-10:00 (30min)" - use estas informações para CALCULAR internamente
     * Ao MENCIONAR para o cliente, informe apenas o horário de INÍCIO (ex: "às 09:30"), não o período completo
     * Informe o intervalo de agendamento do profissional (30 min, 60 min, etc)
...
- CRÍTICO: VERIFICAÇÃO DE DISPONIBILIDADE POR DATA ESPECÍFICA:
  ...
  * Formato de horários ocupados: "OCUPADO 09:30-10:00 (30min), 14:00-15:30 (90min)"
    - Primeiro horário = INÍCIO do agendamento
    - Segundo horário = FIM do agendamento (início + duração)
    - Número entre parênteses = duração total em minutos
  * IMPORTANTE: Ao calcular disponibilidade, considere o PERÍODO COMPLETO (início-fim), não apenas o horário de início
  * Exemplo: Se mostrar "OCUPADO 09:30-10:00 (30min)", significa que TODO o período das 09:30 às 10:00 está ocupado
  * Um serviço de 90min às 09:00 iria de 09:00 até 10:30, então conflitaria com agendamento das 09:30-10:00
  * Se a informação mostrar "LIVRE", o horário está disponível
  * NUNCA confirme horários que se sobreponham a períodos "OCUPADO", "PAUSA" ou "INDISPONÍVEL"
  * Sempre sugira horários alternativos se o solicitado estiver ocupado, em pausa, ou se for dia indisponível
          ...
          if (sendResult.success) {
            ...
            if (isConfirmationResponse && !isRescheduleCtx && !isCancelCtx) {
              ...
              if (currentAIHasAppointmentData) {
                ...
                // Query retorna DESC (mais recentes primeiro), então pegamos as primeiras 10
                const recentMessages = conversationMessages.slice(0, 10);
 ...
 * Só consulta o banco de dados quando realmente necessário
 */
async function getAvailabilityInfoSmart(
  ...
  console.log('📋 Gerando informações de disponibilidade (cache expirado ou primeiro acesso)');
 ...
 */
function clearAvailabilityCache(companyId: number): void {
...
async function getSpecificDateAvailability(
  ...
  for (const prof of professionals) {
    ...
    // Show busy times
    if (dayAppointments.length > 0) {
      const busyTimes = dayAppointments
        .map(apt => apt.appointmentTime)
        .sort()
        .join(', ');
      availabilityText += `  🚫 OCUPADO às: ${busyTimes}\n`;
    } else {
...
async function createAppointmentFromAIConfirmation(conversationId: number, companyId: number, aiResponse: string, phoneNumber: string, initialStatus: string = 'agendado', contactName?: string) {
  try {
    ...
    const recentUserMessages = conversationMessages
      .filter(m => m.role === 'user')
      .slice(0, 3) // First 3 user messages (most recent, query is DESC)
      .map(m => m.content)
      .join(' ');
    ...
    let appointmentDate = getBrazilDate(); // Use Brazil timezone
...
- Lembre-se do que já foi discutido anteriormente na conversa`;
            ...
            if (sendResult.success) {
              ...
              if (isConfirmationResponse && !isReschedCtx && !isCancCtx) {
                ...
                const conversationMessages = await storage.getMessagesByConversation(conversation.id);
                // Query retorna DESC (mais recentes primeiro), então pegamos as primeiras 5
                const recentMessages = conversationMessages.slice(0, 5);

                console.log('📚 Últimas mensagens da conversa (MAIS RECENTE → MAIS ANTIGA):');
                recentMessages.forEach((msg, idx) => {
                  console.log(`  ${idx + 1}. [${msg.role}]: ${msg.content.substring(0, 100)}...`);
                });
