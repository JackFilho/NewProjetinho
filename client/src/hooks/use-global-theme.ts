...
function applyPrimaryColor(root: HTMLElement, colorHex: string) {
...
interface UseGlobalThemeOptions {
  companyPrimaryColor?: string | null;
}

export function useGlobalTheme(options?: UseGlobalThemeOptions) {
  ...
  const companyColor = options?.companyPrimaryColor;

  useEffect(() => {
    if (settings) {
      const root = document.documentElement;

      // Aplica cor primária: prioridade para a cor da empresa, senão usa a global
      const effectivePrimaryColor = companyColor || settings.primaryColor;
      if (effectivePrimaryColor) {
        applyPrimaryColor(root, effectivePrimaryColor);
      }
